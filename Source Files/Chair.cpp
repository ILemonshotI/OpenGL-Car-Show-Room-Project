...
void Chair::setupMesh()
{
    ...
    GLfloat backrestVertices[] = {
        0.36f,0.0f, 0.2f, 0,0,1, 0,0,
         ...
         0.44f,0.7f, 0.2f, 0,0,1, 1,1,
        0.36f,  0.7f, 0.2f, 0,0,1, 0,1,

         0.44f,0.0f, 0.2f, 1,0,0, 0,0,
         0.44f,0.0f,-0.2f, 1,0,0, 1,0,
         0.44f,0.7f,-0.2f, 1,0,0, 1,1,
         0.44f,0.7f, 0.2f, 1,0,0, 0,1,

         0.44f,0.0f,-0.2f, 0,0,-1, 0,0,
        0.36f, 0.0f,-0.2f, 0,0,-1, 1,0,
        0.36f,  0.7f,-0.2f, 0,0,-1, 1,1,
         0.44f,0.7f,-0.2f, 0,0,-1, 0,1,

        0.36f,  0.0f,-0.2f,-1,0,0, 0,0,
        0.36f,  0.0f, 0.2f,-1,0,0, 1,0,
        0.36f,  0.7f, 0.2f,-1,0,0, 1,1,
        0.36f,  0.7f,-0.2f,-1,0,0, 0,1,

        0.36f,  0.7f, 0.2f, 0,1,0, 0,0,
         0.44f,0.7f, 0.2f, 0,1,0, 1,0,
         0.44f,0.7f,-0.2f, 0,1,0, 1,1,
        0.44f, 0.7f,-0.2f, 0,1,0, 0,1,
    ...
    ChairVAO.LinkAttrib(*vbo, 0, 3, GL_FLOAT, 8 * sizeof(float), (void*)0);
    ChairVAO.LinkAttrib(*vbo, 1, 3, GL_FLOAT, 8 * sizeof(float), (void*)(3 * sizeof(float)));
    ChairVAO.LinkAttrib(*vbo, 2, 2, GL_FLOAT, 8 * sizeof(float), (void*)(6 * sizeof(float)));

    ChairVAO.Unbind();
}
void Chair::drawFull(const glm::mat4& view,
    const glm::mat4& proj)
{
    ...
    glm::mat4 chairModel = this->getModelMatrix();
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        Legs[i]->drawWithParent(
            chairModel,  // ? parent model matrix
